/**
 * A bunch of static fuction that do things related to Strings. 
 * @author ADAM
 */
package com.p3.text {
	import flash.utils.Dictionary;

	public class P3StringUtils {
		...
		-------------------------------------------------*/
		/**
		 * Adds extra "0" to a number and returns as a string.
		 * @param	num The number the zeros are being added to.
		 * @param	numZeros the number of "0" to add.
		 * @param	$pos If the "0" are to go to the "left" or "right".
		 * @return	String
		 * @author ADAM
		 */
		public static function padNumber(num : Number, numZeros : Number = 1, $pos : String = "left") : String {
			var _str : String = "";
		...
		public static function padScore(score : int, zeros : uint) : String {
			var str : String = score.toString();
			var zeroStr : String = "";
			for (var i : uint = 0; i < zeros - str.length; ++i)
				zeroStr += "0";
			return zeroStr + str;
		}

		/**
		 * Rounds a number to specified decimal places and returns as a string.
		 * @param	num THe number to be rounded.
		 * @param	numDecimal the number of decimals places to round to.
		 * @return	String
		 * @author http://www.funky-monkey.nl/blog/2009/03/06/how-to-the-big-rounding-numbers-guide-in-actionscript/
		 */
		public static function roundNumber(num : Number, precision : Number = 1) : String {
			if (precision == 0)
		...
		/**
		 * Returns a dictionary contents as a string.
		 * @param	dictionary The dictionary to be printed to a string.
		 * @return String
		 */
		public static function dictionaryToString(dictionary : Dictionary) : String {
			var str : String = "Dictionary = ";
		...
		/**
		 * Returns a string replaceing all the 'delim' with 'replacement';
		 *
		 *	@param p_string The string.
		 *
		 *	@param delim The character to be replaced.
		 *
		 *	@param replacement The string to replace the delim.
		 */
		public static function replaceCharacter(p_string : String, delim : String = "_", replacement : String = " ") : String {
			var reg : RegExp = new RegExp(delim, 'g');
			return p_string.replace(reg, replacement);
		}
		
		/**
		 * Converts a true/false string to boolean (flash coerces ANY string as true). When strict mode
		 * is of any string that isn't "false" returns a true boolean flag.
		 * @param	p_string The string you want to convert
		 * @param	isExact Set to true if you ONLY want exact matches with "true".
		 * @return	a boolean representing if the string was "true" or "false";
		 */
		public static function toBoolean (p_string : String, isExact : Boolean = false):Boolean {
			if (isExact && p_string == "true") return true;
			else if (p_string != "false") return true;
			return false;
		}
		 ...
		 * PRIVATE FUNCTIONS
		-------------------------------------------------*/
		private static function upperCase(p_char : String, ...args) : String {
