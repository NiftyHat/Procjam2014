		...
		public function update():void {
			...
			if (timers != null) {
				...
				for (i = 0; i < timers.length; i++) {
					var curTimer:AxTimer = timers[i];
					if (!curTimer.alive) {
						deadTimers++;
						continue;
					} else if (!curTimer.active) {
						continue;
					}
					curTimer.timer -= Ax.dt;
					while (curTimer.timer <= 0) {
						curTimer.timer += curTimer.delay;
						curTimer.repeat--;
						if (curTimer.callback != null) curTimer.callback();
						if (curTimer.repeat <= 0) {
							curTimer.stop();
							break;
						}
					}
